...
##.SYNOPSIS  
##	   Given basic information found in ServiceNow tickets including server and sitename allow for the manual creation of IIS websites/web 
##	   bindings across all servers in a given farm. Also allows for individual additions of sites without farm deployment. 
##.DESCRIPTION  
##    Given a servers name and a new website site name to be added, user will be prompted for information related to the addition to new website 
##    to a IIS Middleware farm. These include;  server name, credentials, site name, pipeline mode, .NET version, 32 bit encryption status,
##	  basic authentication (on/off), anonymous authentication (on/off) and windows authentication (on/off). The script will also ask which type 
##    of load balancer will be used for the new web site instance and will display the VIP to be used with the site within the environment. After 
##    this information has been obtained the script will test to verify which server farm the server given sits within. If it does not currently 
##    sit in a server farm it will notify user and continue to install the site on the individual box. Otherwise it will prompt the user as to 
##    which farm they will be installing on and then begin installing the site across all servers within the farm. After this step is completed 
##    it will present the user with the VIP information for the farm for their selected VIP type (round robin, sticky or cookie). If the user 
##    selects not to add a new website they will be presented with the ability to add a new webbinding to a already implemented website. The 
##    script will once again ask for the server, site name, if the binding should be SSL and then the web binding that they want added. Upon 
##    entering this information the script will install the binding across either the sites server farm or the sites individual server.  
...
##	  v0.22	    : If a site sits in the dev or test environtment (dv or ts respectively) append -t or -d to the end of the sitename when building
##				  the site. 
##	  v0.23	    : Added functionality to script that checked for dev and test servers and then names site appropriately given this information -d
##                or -t respectively. If a dev or test server no longer keeps added sitename binding ( [sitename] becomes [sitename]-t). 
...
##	  v0.26  	: Updated script to prompt user if their password fails or passes, also will display for pass or fail on initial ping check. 
##	  v0.27  	: Corrected issue where certain users were not able to correctly use automatic SSL certificate binding with HTTPS sites.  
##	  v0.28  	: Made change to have script check and verify that server OS level is 2012 or above before trying to automatically bind certificate to 
