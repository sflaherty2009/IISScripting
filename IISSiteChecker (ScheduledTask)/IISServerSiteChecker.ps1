...
Function funcMergeCSVFiles { 
	...
	$OutputFile = "\\oh0co010\IISWEB\#IISScripting\IISSiteChecker\Logs\WebSiteProperties_$date.csv" 
	$CSVFiles = Get-ChildItem -Path '\\oh0co010\IISWEB\#IISScripting\IISSiteChecker\Logs\Temp' -Name '*.csv'
...
$ServerListFile = "C:\Scripts\IISSiteChecker\ServerList.txt" 
...
if ((Get-Content $ServerListFile) -eq $Null){
	$server = read-host "Please enter the computer to test" 
	Invoke-Command -ComputerName $server -ScriptBlock ${function:funcGetServerWebsites} -ArgumentList $server
