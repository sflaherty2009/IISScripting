...
##.DESCRIPTION  
##    PLEASE NOTE: Include_test.ps1 is the current working include scripting!!! List of functions used in current and future iterations of AEP 
##	  scripts. This FunctionList is a complete set of designed functions and might not work if used "as is". Functions should not be modified 
##	  dramatically in this file as this could break other scripts currently using them in their iterations. Currently local copies of include.ps1 
##    are stored on users local computers to bypass the second hop limitation experienced by Powershell scripting. include.ps1 is currently used 
##    for all IIS Tools including; WebcontentChange, IISInstall, IISWebSiteInstall and IISWebSiteRemoval. PatchFix.ps1 does not use this include 
##    file and is currently on its own include.ps1. 
##.NOTES 
##    FileName	 :_FunctionList_v0.18.ps1 
	  $Lversion = 'v0.38'
	  $LIISInstallVersion = '0.09'
	  $LIISWebsiteConfigVersion = '0.07'
	  $LIISWebsiteInstallVersion = '0.31'
	  $LIISWebsiteRemovalVersion = '0.12'
	  $LLauncherVersion = '0.03'
...
##					"\\hqcelfs01\complex_servers\Windows\external\Project_Tracking\tbl_cmdb_ci_IOSndows.csv"
...
##	  v0.19 	: Added all scripts to be updated by FuncCheckcurrentVersion.
##	  v0.20 	: Added central repository for all scripts to call on when checking for saved username and password.
##	  v0.21 	: Corrected issue where FuncUserPassword was not testing correct path for updates. 
##	  v0.22 	: Corrected issue current scripts weren't being overwritten because the were already in place. Added -force parameter
##				  to copy-item command
##	  v0.23 	: Added section into FuncUserPassword to correctly identify userID.  
##	  v0.24 	: Added section into FuncDomainUser to pull from new credentials path. 
##	  v0.25 	: Added ability for LauncherVersion to be updated through tool. 
##	  v0.26 	: Added delay to webcontent folder deployment. This delay will allow for the webcontent folder to be available to servers
##				  before trying to add webconfig. 
##	  v0.27 	: Corrected issue with application path testing. While try catch was working correctly the variable being called on 
##				  TestAppPath was not properly implemented.
##	  v0.28 	: Found missing function FunChangeWebSettings. Found Function in past trial code and reimplemented in include_test. Tested
##				  and verified working. 
##	  v0.29 	: Corrected log location for IISInstall function of script. Now statically set to point to the IISINstall/Logs folder location. 
##	  v0.30 	: Corrected issue where authentication methods were getting placed in web.config instead of applicationhost.config. Added 
##				  commit:apphost
##	  v0.31 	: Corrected issue where authentication methods were being disabled in web.config but not apphost.config. Added commit:apphost to 
##				  disable property in both websiteadd function and changewebsite function 
##	  v0.31.1 	: Corrected issue where basic authentication was not being set correctly due to error in if/then statement.
##	  v0.32 	: Added additional functionality to FuncChangeWebSettings to allow for configuration changes to be turned on and off so not all 
##				  prior configurations would need to be known in order to use script.
##	  v0.33 	: Updated FuncheckCurrentVersion to remove outdated script before replacing the script with a new copy. 
##	  v0.33.1 	: Updated base .ps1 for IISWebsiteConfig_test.ps1 to v0.05, redeploying. 
##	  v0.33.2 	: Updated FuncCheckCurrentVersion to delete old folder correctly.  
##	  v0.33.3 	: Incorrectly using Remove-Item functionality in FuncCheckCurrentVersion. Updated with appropriate syntax.   
##	  v0.33.4 	: Accidently removed -recurse off of Remove-Item syntax that was needed. Corrected error.  
##	  v0.34 	: Found syntax error in FunChangeWebSettings /enabled:false /commit:apphost was missing proper spacing between false and /commit 
##				  and was failing. Corrected issue. 
##	  v0.35 	: Path being called for dotnetfx40_x64_x86.exe and NET_40_Installation.xml was not appropriate for updated script. Set paths 
##				  statically to work in new architecture. 
##	  v0.35.1 	: Updated IISWEbsiteConfig to v0.06. 
##	  v0.36 	: Removed $path component to include.ps1. Now statically set to C:\Scripts\. This is due to script being installed and not moved  
##				  from place to place.  
##	  v0.36.1 	: Took FuncWebContentCheck out of testing phase.  
##	  v0.37 	: changed the FuncCheckCurrentVersion to allow display version numbers if the number was out of date and would be updated. 
##	  v0.37.1 	: Updated the called functions to return to launcher after being run and completed.  
##	  v0.37.2 	: Updated Launcher.ps1 to allow for a quit process prompt in its prompt collection.
##	  v0.38 	: Corrected an issue where displayed TCPIP information, for ticket creation was not displaying the appropriate WebBinding but the 
##			      siteName.
...
#(TESTING)check the current version all IIS Tools in the IISScripting folder. 
Function FuncCheckCurrentVersion{
	...
	try{
		$xml = [XML](Get-Content "\\sfsdwdc004\dtapdata\Software Engineering\Middleware\#IISScripting\_Lib\Variables.xml")
	}
	...
	#Lets test our path to verify if we have a place to put our scripts. If not lets build a place for them.
	if (!(Test-Path "C:\scripts")){
		try{
			...
			write-warning "Could not create container for IIS Scripts."
		}
	}
	...
	$IISInstallVersion = $xml.variables.IISInstallVersion
	$IISWebsiteConfigVersion = $xml.variables.IISWebsiteConfigVersion
	$IISWebsiteInstallVersion = $xml.variables.IISWebsiteInstallVersion
	$IISWebsiteRemovalVersion = $xml.variables.IISWebsiteRemovalVersion
	$LauncherVersion = $xml.variables.LauncherVersion
	
	#display versions of all my current scripts and their local copy versions. 
	if ($currentVersion -ne $Lversion){
		write-host "Current Include.ps1 Version" -foregroundcolor yellow 
		write-host $currentVersion 
		write-host "Current Installed Include.ps1 Version" -foregroundcolor yellow 
		write-host $Lversion
	}
	if ($IISInstallVersion -ne $LIISInstallVersion){
		write-host "Current IISInstall Version" -foregroundcolor yellow 
		write-host $IISInstallVersion 
		write-host "Current Installed IISInstall Version" -foregroundcolor yellow 
		write-host $LIISInstallVersion
	}
	
	if ($IISWebsiteConfigVersion -ne $LIISWebsiteConfigVersion){
		write-host "Current IISWebsiteConfig Version" -foregroundcolor yellow 
		write-host $IISWebsiteConfigVersion 
		write-host "Current Installed IISWebsiteConfig Version" -foregroundcolor yellow 
		write-host $LIISWebsiteConfigVersion
	}
	
	if ($IISWebsiteInstallVersion -ne $LIISWebsiteInstallVersion){
		write-host "Current IISWebsiteInstall Version" -foregroundcolor yellow 
		write-host $IISWebsiteInstallVersion
		write-host "Current Installed IISWebsiteInstall Version" -foregroundcolor yellow 
		write-host $LIISWebsiteInstallVersion
	}
	if ($IISWebsiteRemovalVersion -ne $LIISWebsiteRemovalVersion){
		write-host "Current IISWebsiteRemoval Version" -foregroundcolor yellow 
		write-host $IISWebsiteRemovalVersion
		write-host "Current Installed IISWebsiteRemoval Version" -foregroundcolor yellow 
		write-host $LIISWebsiteRemovalVersion
	}
	
	if ($LauncherVersion -ne $LLauncherVersion){
		write-host "Current Launcher Version" -foregroundcolor yellow 
		write-host $LauncherVersion
		write-host "Current Installed Launcher Version" -foregroundcolor yellow 
		write-host $LLauncherVersion
	}
	
	if (Test-Path "C:\scripts"){
		#If the current version is not the same as the version on our computer update our scripts. 
		if ($currentVersion -ne $Lversion){
			...
			$sourcePath = "\\sfsdwdc004\dtapdata\Software Engineering\Middleware\#IISScripting\_Lib\include_test.ps1"
			#place it on our computer. 
			$destPath = "C:\scripts\_lib\include_test.ps1"
		...
		if ($IISInstallVersion -ne $LIISInstallVersion){
			write-host "updating your version of IISInstall to $IISInstallVersion" -foregroundcolor yellow 
			#Import-Module bitstransfer
			#get it from the network share. 
			$sourcePath = "\\sfsdwdc004\dtapdata\Software Engineering\Middleware\#IISScripting\IISInstall"
			#place it on our computer. 
			$destPath = "C:\scripts\IISInstall"
		...
		if ($IISWebsiteConfigVersion -ne $LIISWebsiteConfigVersion){
			write-host "updating your version of IISWebsiteConfig to $IISWebsiteConfigVersion" -foregroundcolor yellow 
			#Import-Module bitstransfer
			#get it from the network share. 
			$sourcePath = "\\sfsdwdc004\dtapdata\Software Engineering\Middleware\#IISScripting\IISWebsiteConfig"
			#place it on our computer. 
			$destPath = "C:\scripts\IISWebsiteConfig"
		...
		if ($IISWebsiteInstallVersion -ne $LIISWebsiteInstallVersion){
			write-host "updating your version of IISWebsiteInstall to $IISWebsiteInstallVersion" -foregroundcolor yellow 
			#Import-Module bitstransfer
			#get it from the network share. 
			$sourcePath = "\\sfsdwdc004\dtapdata\Software Engineering\Middleware\#IISScripting\IISWebsiteInstall"
			#place it on our computer. 
			$destPath = "C:\scripts\IISWebsiteInstall"
		...
		if ($IISWebsiteRemovalVersion -ne $LIISWebsiteRemovalVersion){
			write-host "updating your version of IISWebsiteRemoval to $IISWebsiteRemovalVersion" -foregroundcolor yellow 
			#Import-Module bitstransfer
			#get it from the network share. 
			$sourcePath = "\\sfsdwdc004\dtapdata\Software Engineering\Middleware\#IISScripting\IISWebsiteRemoval"
			#place it on our computer. 
			$destPath = "C:\scripts\IISWebsiteRemoval"
		...
		if ($LauncherVersion -ne $LLauncherVersion){
			write-host "updating your version of Launcher to $LauncherVersion" -foregroundcolor yellow 
			#Import-Module bitstransfer
			#get it from the network share. 
			$sourcePath = "\\sfsdwdc004\dtapdata\Software Engineering\Middleware\#IISScripting\_lib\Launcher.ps1"
			#place it on our computer. 
			$destPath = "C:\scripts\_lib\Launcher.ps1"
		...
		write-warning "Script location C:\scripts does not exist scipt cannot continue."
...
Function FuncTranscript([switch]$debug){
 ...
 if(([datetime]::now.toLocalTIme()) -match "AM") 
   ...
   write-debug "Starting transcript ... C:\scripts\Logs\LogFile_$date.txt"
 start-transcript -path "C:\scripts\Logs\LogFile_$date.txt" -append
 } #end start-Trans
...
Function FuncCheckConnectivity($Server,[switch]$Remember){
	...
	if (!(test-path "C:\scripts\Logs\error_Lists")){
		try{
			New-Item -Path "C:\scripts\Logs\error_Lists" -type directory
		}
		catch{
			write-warning "Cannot create path C:\scripts\Logs\error_Lists for logging of Connectivity failures"
		}
	}
    ...
    $PingCheck = {
        ...
        if (Test-Connection -ComputerName $Server -BufferSize 16 -Count 1 -ErrorAction 0 -quiet) { # ErrorAction 0 doesn't display error information when a ping is unsuccessful
            ...
            if (!$?) {
                ...
                $Server | Add-Content "C:\scripts\Logs\error_Lists\DNS_Fail_$date.txt"
                ...
                if (Test-Connection -ComputerName $Server -BufferSize 16 -Count 1 -ErrorAction 0 -Quiet) {
                   ...
                   return
                      ...
                      $ping = "Fail"
                      $global:DNSCheck = "Fail"
                      $Server | Add-Content "C:\scripts\Logs\error_Lists\Ping_Fail_$date.txt"
...
Function FuncCheckRDP($Server, $Credent){
	...
	if (!(test-path "C:\scripts\Logs\error_Lists")){
		try{
			...
			write-warning "Cannot create path C:\scripts\Logs\error_Lists for logging of RDP Connectivity failures"
		}
	}
	...
	try {
		...
		try {
			...
			try{FuncEnableRemoting -server $server 
				try{
					...
					$Server | Add-Content "C:\scripts\Logs\error_Lists\Credentials_Fail_$date.txt"
					return ,$RDP 
				}
			}
			catch{
		...
		$RDP = "Fail"
		$Server | Add-Content "C:\scripts\Logs\error_Lists\RDP_Fail_$date.txt"
		$global:RDPCheck = "Fail"
		return ,$RDP
...
Function FuncUserPassword{
	if (!(Test-Path "C:\scripts\_credentials")){
		try{
			New-Item -Path "C:\scripts\_credentials" -type directory
		
		}
		catch{
			write-warning "Could not create container for Credentials."
		}
	}
	
	if ((Get-Content "C:\scripts\_credentials\$userid'_IOS.txt'") -eq $Null){
		Read-Host "Please enter your IOS account password" -assecurestring | ConvertFrom-SecureString | out-file "C:\scripts\_credentials\$userid'_IOS.txt'"

		}
	if ((Get-Content "C:\scripts\_credentials\$userid'_DOCUTAP.txt'") -eq $Null){
		Read-Host "Please enter your DOCUTAP account password" -assecurestring | ConvertFrom-SecureString | out-file "C:\scripts\_credentials\$userid'_DOCUTAP.txt'"
...
Function FuncDomainUser ($server){	
		...
		if($domain -eq "IOS"){
			$username = "IOS\$userid"
			$password = get-content C:\scripts\_credentials\$userid'_IOS.txt' | convertto-securestring	
			$cred = new-object -typename System.Management.Automation.PSCredential -ArgumentList $username,$password
			return, $cred
		}
		if($domain -like "DOCUTAP"){
			$username = "DOCUTAP\$userid"
			$password = get-content C:\scripts\_credentials\$userid'_DOCUTAP.txt' | convertto-securestring
			$cred = new-object -typename System.Management.Automation.PSCredential -ArgumentList $username,$password
			return, $cred
		}
	...
	if ($rdpCheck -eq "Pass" -and $pingCheck -eq "Pass"){
		...
		if ($global:flagContentFolder -ne "f"){
			...
			if (!(test-path $testPath)){
				...
				write-host "waiting for new webcontent folder $sitename to deploy (this process can take up to 30 seconds)"
				Start-Sleep -s 30
			}
		...
		if ($siteExists -ne "True" -and $appExists -ne "True"){
			...
			if ($bAuth -eq "true"){
				...
				Invoke-Command -ComputerName $server -credential $cred -Command{param($siteName) c:\Windows\System32\inetsrv\appcmd.exe set config "$sitename" /section:basicAuthentication /enabled:true /commit:apphost} -ArgumentList $siteName
			}
			...
			if ($wAuth -eq "true"){
				...
				#enable authentication
				Invoke-Command -ComputerName $server -credential $cred -Command{param($siteName) c:\Windows\System32\inetsrv\appcmd.exe set config "$sitename" /section:windowsAuthentication /enabled:true /commit:apphost} -ArgumentList $siteName
			}
			...
			if ($aAuth -eq "true"){
				...
				#enable authentication
				Invoke-Command -ComputerName $server -credential $cred -Command{param($siteName) c:\Windows\System32\inetsrv\appcmd.exe set config "$sitename" /section:anonymousAuthentication /enabled:true /commit:apphost} -ArgumentList $siteName
			}
			if ($aAuth -ne "true"){
				Invoke-Command -ComputerName $server -credential $cred -Command{param($siteName) c:\Windows\System32\inetsrv\appcmd.exe unlock config /section:anonymousAuthentication}
				#disable authentication for both apphost config and web.config
				...
				Invoke-Command -ComputerName $server -credential $cred -Command{param($siteName) c:\Windows\System32\inetsrv\appcmd.exe set config "$sitename" /section:anonymousAuthentication /enabled:false /commit:apphost} -ArgumentList $siteName
			} 
		}
		else{
		...
		if ($vipName -eq '' -or $vipName -eq $null){
			...
			$aliases = "$siteNameBinding.aepsc.com"
			write-host "nslookup      $siteNameBinding"
		...
		if ($application -eq 'true'){
			$testAppPath = Invoke-Command -ComputerName $server -credential $cred -Command{param($appPath)test-path $appPath} -ArgumentList $appPath
...
#Determines the VIP location for given server farm whether sticky, round robin or cookie. 
...
Function FuncUserNamePassword ($server){
	...
	if (((Get-Content C:\scripts\Credentials\$userid'_IOS.txt') -eq $Null) -or ((Get-Content C:\scripts\Credentials\$userid'_DOCUTAP.txt') -eq $Null)){
		if ((Get-Content C:\scripts\Credentials\$userid'_IOS.txt') -eq $Null){
			Read-Host "Please enter your WEBINT account password" -assecurestring | ConvertFrom-SecureString | out-file C:\scripts\Credentials\$userid'_IOS.txt'

			}
		if ((Get-Content C:\scripts\Credentials\$userid'_DOCUTAP.txt') -eq $Null){
			Read-Host "Please enter your CORPTEST account password" -assecurestring | ConvertFrom-SecureString | out-file C:\scripts\Credentials\$userid'_DOCUTAP.txt'
		...
		if($domain -eq "IOS"){
			$username = "IOS\$userid"
			$password = get-content C:\scripts\Credentials\$userid'_IOS.txt' | convertto-securestring	
			$cred = new-object -typename System.Management.Automation.PSCredential -ArgumentList $username,$password
			return, $cred
		}
		if($domain -like "DOCUTAP"){
			$username = "DOCUTAP\$userid"
			$password = get-content C:\scripts\Credentials\$userid'_DOCUTAP.txt' | convertto-securestring
			$cred = new-object -typename System.Management.Automation.PSCredential -ArgumentList $username,$password
			return, $cred
		}
		if($domain -eq $null){
	...
	if ($rdpCheck -eq "Pass" -and $pingCheck -eq "Pass" -and $driveCheck -ne "Fail" )
	{
		...
		if ($OSCheck -match "Server 2008"){
			...
			try{
				...
				$sourcePath = "C:\scripts\IISInstall\lib\Scripts\dotnetfx40_x64_x86.exe"
				$destPath = "\\$server\C$\script\dotnetfx40_x64_x86.exe"
				Start-BitsTransfer -Source $sourcePath -Destination $destPath -Credential $cred
				$sourcePath = "C:\scripts\IISInstall\lib\Scripts\NET_40_Installation.xml"
...
Function FuncCheckIIS ($server, $cred){
	...
	Add-Content "C:\scripts\IISInstall\Logs\LogFile_$server.txt" "LogFile available on D drive ::"
	$logPath | Add-Content "C:\scripts\IISInstall\Logs\LogFile_$server.txt"
	Add-Content "C:\scripts\IISInstall\Logs\LogFile_$server.txt" "WebContent available on D drive :: "
	$webPath | Add-Content "C:\scripts\IISInstall\Logs\LogFile_$server.txt"
	Add-Content "C:\scripts\IISInstall\Logs\LogFile_$server.txt" "LogLocation fowarded to D:/Logfile ::"
	$logLocation | Add-Content "C:\scripts\IISInstall\Logs\LogFile_$server.txt"
	Add-Content "C:\scripts\IISInstall\Logs\LogFile_$server.txt" "local TimeValue set for default sites"
	$timeValue | Add-Content "C:\scripts\IISInstall\Logs\LogFile_$server.txt"
	Add-Content "C:\scripts\IISInstall\Logs\LogFile_$server.txt" "IIS Components currently installed ::"
	$IISInstall | Select DisplayName, Installed | Add-Content "C:\scripts\IISInstall\Logs\LogFile_$server.txt"
	
	try{
		Import-Module bitstransfer
		$sourcePath = "C:\scripts\IISInstall\Logs\LogFile_$server.txt"
...
Function FuncDeleteIISWebsite ($server, $siteName){
	...
	if ($OSCheck -match "Server 2003"){
		...
		if ($pingCheck -eq "Pass" ){
			...
			$hashmap = @{}
			$PathMatch = @()
			$vbScriptPath = "\\sfsdwdc004\dtapdata\Software Engineering\Middleware\#IISScripting\IISWebsiteRemoval\lib\adsutil.vbs"
			...
			if (!(test-path "\\$server\C$\script\$scriptName")){
				#Import-Module bitstransfer
				#get it from the network share. 
				$sourcePath = "\\sfsdwdc004\dtapdata\Software Engineering\Middleware\#IISScripting\IISWebsiteRemoval\lib\$scriptName"
...
#Run Cleanup.bat file in order to clean C drive if noted that it does not have enough space. 
Function FuncRunScriptFile ($server,$scriptName,$credent){
	...
	Import-Module bitstransfer
	$sourcePath = "C:\scripts\lib\Scripts\$scriptName"
	$destPath = "\\$server\C$\script\$scriptName"
...
Function FuncRunLanDesk ($server,$credent){
	...
	#import file to script folder within server in question 
	Import-Module bitstransfer
	$sourcePath = "C:\scripts\lib\Scripts\CurrentPatchx64.cmd"
...
Function FuncCheckDC ($server){
	...
	if ($dcMatch -ne $Null){
		$DC = "Detected" 
		$Server | Add-Content "C:\scripts\Logs\error_Lists\DC_Detected_$date.txt"
		return ,$DC
...
Function FuncWinUPCheckService ($Server, $Credent){
	...
	if ($arrService.Status -ne "Running" -and $count -gt 1){
		$WUpdate = "Failed"
		$Server | Add-Content "C:\scripts\Logs\error_Lists\WindowsUpdate_Service_Fail_$date.txt"
		return, $WUpdate
...
Function FuncApplicationVersion ($Server, $AppName, $Version, $Credent){
	...
	if ($arrService.version -lt $Version){
		...
		Add-Content -path "C:\scripts\Logs\error_Lists\LanDesk_Fail_$date.txt" -value "$server - $installedVersion"
		return ,$AVersion
	} 
	else {
		$AVersion = $arrService.version 
	...
	if ($diskStats -eq $null)
	{
		...
		if ($fs  -lt $driveSpace){
			...
			if ($fs  -lt $driveSpace){
				...
				$Server | Add-Content "C:\scripts\Logs\error_Lists\CDrive_Fail_$date.txt"
				return ,$DSpace
			}
...
#(TESTING) function used to change the configuration on sites that have already been pushed to the farm.
Function FunChangeWebSettings($server, $cred, $siteName, $pipeline, $version, $bit, $bAuth, $aAuth, $wAuth, $Auth, $application, $appName, $appPath){
	...
	#if the server can be reached we can continue our script
	if ($rdpCheck -eq "Pass" -and $pingCheck -eq "Pass"){
		#verify that our site and application exists otherwise we aren't really changing permissions are we? 
		if ($siteExists -eq "True" -and $appExists -eq "True"){
	#CONFIGURATION ----------------------------------------------	
			#change application pool with Runtime version (v2.0/v4.0), pipeline mode (Integrated/Classic), 32Bit mode (true/false)
			if ($version -ne 'off'){	
				#DEPRECATED -- command now sectionalized to allow functions to be turned off. 
				#Invoke-Command -ComputerName $server -credential $cred -Command{param($siteName, $version, $pipeline, $bit) c:\Windows\System32\inetsrv\appcmd.exe set apppool /apppool.name:$sitename /managedRuntimeVersion:$version /managedPipelineMode:$pipeline /enable32BitAppOnWin64:$bit } -ArgumentList $siteName, $version, $pipeline, $bit
				
				Invoke-Command -ComputerName $server -credential $cred -Command{param($siteName, $version) c:\Windows\System32\inetsrv\appcmd.exe set apppool /apppool.name:$sitename /managedRuntimeVersion:$version} -ArgumentList $siteName, $version
				
			}
			if ($bit -ne 'off'){
				Invoke-Command -ComputerName $server -credential $cred -Command{param($siteName, $bit) c:\Windows\System32\inetsrv\appcmd.exe set apppool /apppool.name:$sitename /enable32BitAppOnWin64:$bit } -ArgumentList $siteName, $bit
			
			}
			
			if ($pipeline -ne 'off'){
				Invoke-Command -ComputerName $server -credential $cred -Command{param($siteName, $pipeline) c:\Windows\System32\inetsrv\appcmd.exe set apppool /apppool.name:$sitename /managedPipelineMode:$pipeline} -ArgumentList $siteName, $pipeline
			
			}
	
	#APPLICATION--------------------------------------------------
			#lets check the path to our test web application and verify its in place. 
			$testAppPath = Invoke-Command -ComputerName $server -credential $cred -Command{param($appPath)test-path $appPath} -ArgumentList $appPath
	
			#add application to existing site if they user indicated they wanted to. 
			if ($application -ne 'off'){
				if ($application -eq 'true'){
					#hey our application does have a folder location lets go ahead and add it. 
					if (($testAppPath -eq "true")){
						Invoke-Command -ComputerName $server -credential $cred -Command{param($appName, $appPath, $siteName) import-module webadministration; cd IIS:\; New-Item -Path "IIS:\Sites\$siteName\$appName" -PhysicalPath $appPath -type Application} -ArgumentList $appName, $appPath, $siteName
	...
	#AUTHENTICATION -----------------------------------------------
			...
			#We will need the ability to turn authentication on and off since it could already be on with initial setup. 
			...
			if ($auth -ne 'off'){
				...
				if ($bAuth -ne "true"){
					Invoke-Command -ComputerName $server -credential $cred -Command{param($siteName) c:\Windows\System32\inetsrv\appcmd.exe unlock config /section:basicAuthentication}
					#disable authentication for both apphost config and web.config.
					Invoke-Command -ComputerName $server -credential $cred -Command{param($siteName) c:\Windows\System32\inetsrv\appcmd.exe set config "$sitename" /section:basicAuthentication /enabled:false} -ArgumentList $siteName
					Invoke-Command -ComputerName $server -credential $cred -Command{param($siteName) c:\Windows\System32\inetsrv\appcmd.exe set config "$sitename" /section:basicAuthentication /enabled:false /commit:apphost} -ArgumentList $siteName
				}
				...
				if ($wAuth -ne "true"){
					...
					Invoke-Command -ComputerName $server -credential $cred -Command{param($siteName) c:\Windows\System32\inetsrv\appcmd.exe unlock config /section:windowsAuthentication}
					#disable authentication for both apphost config and web.config. 
					Invoke-Command -ComputerName $server -credential $cred -Command{param($siteName) c:\Windows\System32\inetsrv\appcmd.exe set config "$sitename" /section:windowsAuthentication /enabled:false} -ArgumentList $siteName
					Invoke-Command -ComputerName $server -credential $cred -Command{param($siteName) c:\Windows\System32\inetsrv\appcmd.exe set config "$sitename" /section:windowsAuthentication /enabled:false /commit:apphost} -ArgumentList $siteName
				}
		...
		write-host "site $siteName does not exist on server $server. Please verify site and try again" -foregroundcolor red
...
#(DONE)Update all functions to use statically set C:\scripts\ path for logging and checks. 